...
# -----LSL Start------
info = StreamInfo('PsychoPy_Markers', 'Markers', 1, 0, 'string', 'myuidw43536')
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/tonysaldana/Downloads/Clean/BT1_Psychopy.py',
...
def setupWindow(expInfo=None, win=None):
    ...
    if win is None:
        # ---> START PRE-FLIGHT DIALOG <---
        dlg = gui.Dlg(title="LabRecorder Pre-Flight")
        dlg.addText("The LSL Stream is now broadcasting in the background!")
        dlg.addText("")
        dlg.addText("1. Open LabRecorder")
        dlg.addText("2. Click 'Update', check the stream box, and hit 'Start'")
        dlg.addText("3. Come back here and click 'OK' to start the experiment.")
        dlg.show()
        
        if not dlg.OK:
            core.quit()
        # ---> END PRE-FLIGHT DIALOG <---
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('Transition_to_Practice') is None:
        # initialise Transition_to_Practice
        Transition_to_Practice = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='Transition_to_Practice',
        )
...
def pauseExperiment(thisExp, win=None, timers=[], currentsline=None):
    """
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Introduction_text = visual.TextStim(win=win, name='Introduction_text',
        ...
        languageStyle='LTR',
        depth=0.0);
    Jaw_Clench_Text = visual.TextStim(win=win, name='Jaw_Clench_Text',
        text='We will now do a jaw clench to align the data.\nWhen you see the triangle on the screen clench your jaw. We will do this three times.\nWhen you are ready to start press space.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # --- Initialize components for Routine "JawClench_Alignment" ---
    Get_Ready = visual.TextStim(win=win, name='Get_Ready',
        ...
        languageStyle='LTR',
        depth=0.0);
    Jaw_Clench = visual.ShapeStim(
        win=win, name='Jaw_Clench',
        size=(0.5, 0.5), vertices='triangle',
        ori=0.0, pos=(0, 0), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
        opacity=None, depth=-1.0, interpolate=True)
    Jaw_Clench_2 = visual.ShapeStim(
        win=win, name='Jaw_Clench_2',
        size=(0.5, 0.5), vertices='triangle',
        ori=0.0, pos=(0, 0), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
        opacity=None, depth=-2.0, interpolate=True)
    Jaw_Clench_3 = visual.ShapeStim(
        win=win, name='Jaw_Clench_3',
        size=(0.5, 0.5), vertices='triangle',
        ori=0.0, pos=(0, 0), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
    ...
    Practice_Trial_Text = visual.TextStim(win=win, name='Practice_Trial_Text',
        text='We will now begin the practice session.\nYou will focus on the dot in the middle.\nYou will close your eyes first, so once you press space you will hear a beep to close your eyes. When you hear a beep again, you open your eyes.\nYou will do this four times, so four beeps.\nTo begin the practice session press the space bar.\n',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-4.0);
    Transition_to_Practice = keyboard.Keyboard(deviceName='Transition_to_Practice')
    
    # --- Initialize components for Routine "Practice_Session" ---
    PT_Dot_EC = visual.ImageStim(
        win=win,
        name='PT_Dot_EC', 
        image='/Users/tonysaldana/Downloads/Neurotech 25-26/ProstheticArm/Images&Sounds/Black_Dot.png', mask=None, anchor='center',
    ...
    # routine timer to track time remaining of each (possibly non) routine
    ...
    Introduction = data.Routine(
        name='Introduction',
        components=[Introduction_text, Jaw_Clench_Text, Intro_Key_Resp],
    )
    ...
    while continueRoutine:
        ...
        if Introduction_text.status == STARTED:
            ...
            if tThisFlipGlobal > Introduction_text.tStartRefresh + 25.0-frameTolerance:
        ...
        # *Jaw_Clench_Text* updates
        
        # if Jaw_Clench_Text is starting this frame...
        if Jaw_Clench_Text.status == NOT_STARTED and tThisFlip >= 25.0-frameTolerance:
            # keep track of start time/frame for later
            Jaw_Clench_Text.frameNStart = frameN  # exact frame index
            Jaw_Clench_Text.tStart = t  # local t and not account for scr refresh
            Jaw_Clench_Text.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Jaw_Clench_Text, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Jaw_Clench_Text.started')
            # update status
            Jaw_Clench_Text.status = STARTED
            Jaw_Clench_Text.setAutoDraw(True)
        
        # if Jaw_Clench_Text is active this frame...
        if Jaw_Clench_Text.status == STARTED:
            # update params
            pass
        ...
        if Intro_Key_Resp.status == NOT_STARTED and tThisFlip >= 25.0-frameTolerance:
            # keep track of start time/frame for later
    ...
    # --- Prepare to start Routine "JawClench_Alignment" ---
    # create an object to store info about Routine JawClench_Alignment
    JawClench_Alignment = data.Routine(
        name='JawClench_Alignment',
        components=[Get_Ready, Jaw_Clench, Jaw_Clench_2, Jaw_Clench_3, Practice_Trial_Text, Transition_to_Practice],
    )
    JawClench_Alignment.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # create starting attributes for Transition_to_Practice
    Transition_to_Practice.keys = []
    Transition_to_Practice.rt = []
    _Transition_to_Practice_allKeys = []
    # store start times for JawClench_Alignment
    JawClench_Alignment.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    JawClench_Alignment.tStart = globalClock.getTime(format='float')
    JawClench_Alignment.status = STARTED
    thisExp.addData('JawClench_Alignment.started', JawClench_Alignment.tStart)
    JawClench_Alignment.maxDuration = None
    # keep track of which components have finished
    JawClench_AlignmentComponents = JawClench_Alignment.components
    for thisComponent in JawClench_Alignment.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "JawClench_Alignment" ---
    JawClench_Alignment.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # if Get_Ready is starting this frame...
        if Get_Ready.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Get_Ready.started')
            # update status
            Get_Ready.status = STARTED
            Get_Ready.setAutoDraw(True)
        
        # if Get_Ready is active this frame...
        if Get_Ready.status == STARTED:
            # update params
            pass
        
        # if Get_Ready is stopping this frame...
        if Get_Ready.status == STARTED:
            # is it time to stop? (based on local clock)
            if tThisFlip > 5.0-frameTolerance:
                # keep track of stop time/frame for later
                Get_Ready.tStop = t  # not accounting for scr refresh
                Get_Ready.tStopRefresh = tThisFlipGlobal  # on global time
                Get_Ready.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Get_Ready.stopped')
                # update status
                Get_Ready.status = FINISHED
                Get_Ready.setAutoDraw(False)
        
        # *Jaw_Clench* updates
        
        # if Jaw_Clench is starting this frame...
        if Jaw_Clench.status == NOT_STARTED and tThisFlip >= 5.0-frameTolerance:
            # keep track of start time/frame for later
            Jaw_Clench.frameNStart = frameN  # exact frame index
            Jaw_Clench.tStart = t  # local t and not account for scr refresh
            Jaw_Clench.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Jaw_Clench, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Jaw_Clench.started')
            # update status
            Jaw_Clench.status = STARTED
            Jaw_Clench.setAutoDraw(True)
            
            marker_outlet.push_sample(['JawClench'])
        
        # if Jaw_Clench is active this frame...
        if Jaw_Clench.status == STARTED:
            # update params
            pass
        
        # if Jaw_Clench is stopping this frame...
        if Jaw_Clench.status == STARTED:
            # is it time to stop? (based on local clock)
            if tThisFlip > 9.0-frameTolerance:
                # keep track of stop time/frame for later
                Jaw_Clench.tStop = t  # not accounting for scr refresh
                Jaw_Clench.tStopRefresh = tThisFlipGlobal  # on global time
                Jaw_Clench.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Jaw_Clench.stopped')
                # update status
                Jaw_Clench.status = FINISHED
                Jaw_Clench.setAutoDraw(False)
        
        # *Jaw_Clench_2* updates
        
        # if Jaw_Clench_2 is starting this frame...
        if Jaw_Clench_2.status == NOT_STARTED and tThisFlip >= 11.0-frameTolerance:
            # keep track of start time/frame for later
            Jaw_Clench_2.frameNStart = frameN  # exact frame index
            Jaw_Clench_2.tStart = t  # local t and not account for scr refresh
            Jaw_Clench_2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Jaw_Clench_2, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Jaw_Clench_2.started')
            # update status
            Jaw_Clench_2.status = STARTED
            Jaw_Clench_2.setAutoDraw(True)
            
            marker_outlet.push_sample(['JawClench'])
        
        # if Jaw_Clench_2 is active this frame...
        if Jaw_Clench_2.status == STARTED:
            # update params
            pass
        
        # if Jaw_Clench_2 is stopping this frame...
        if Jaw_Clench_2.status == STARTED:
            # is it time to stop? (based on local clock)
            if tThisFlip > 15.0-frameTolerance:
                # keep track of stop time/frame for later
                Jaw_Clench_2.tStop = t  # not accounting for scr refresh
                Jaw_Clench_2.tStopRefresh = tThisFlipGlobal  # on global time
                Jaw_Clench_2.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Jaw_Clench_2.stopped')
                # update status
                Jaw_Clench_2.status = FINISHED
                Jaw_Clench_2.setAutoDraw(False)
        
        # *Jaw_Clench_3* updates
        
        # if Jaw_Clench_3 is starting this frame...
        if Jaw_Clench_3.status == NOT_STARTED and tThisFlip >= 17.0-frameTolerance:
            # keep track of start time/frame for later
            Jaw_Clench_3.frameNStart = frameN  # exact frame index
            Jaw_Clench_3.tStart = t  # local t and not account for scr refresh
            Jaw_Clench_3.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Jaw_Clench_3, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Jaw_Clench_3.started')
            # update status
            Jaw_Clench_3.status = STARTED
            Jaw_Clench_3.setAutoDraw(True)
            
            marker_outlet.push_sample(['JawClench'])
        
        # if Jaw_Clench_3 is active this frame...
        if Jaw_Clench_3.status == STARTED:
            # update params
            pass
        
        # if Jaw_Clench_3 is stopping this frame...
        if Jaw_Clench_3.status == STARTED:
            # is it time to stop? (based on local clock)
            if tThisFlip > 21.0-frameTolerance:
                # keep track of stop time/frame for later
                Jaw_Clench_3.tStop = t  # not accounting for scr refresh
                Jaw_Clench_3.tStopRefresh = tThisFlipGlobal  # on global time
                Jaw_Clench_3.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Jaw_Clench_3.stopped')
                # update status
                Jaw_Clench_3.status = FINISHED
                Jaw_Clench_3.setAutoDraw(False)
        
        # *Practice_Trial_Text* updates
        
        # if Practice_Trial_Text is starting this frame...
        if Practice_Trial_Text.status == NOT_STARTED and tThisFlip >= 23.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *Transition_to_Practice* updates
        waitOnFlip = False
        
        # if Transition_to_Practice is starting this frame...
        if Transition_to_Practice.status == NOT_STARTED and tThisFlip >= 23.0-frameTolerance:
            # keep track of start time/frame for later
            Transition_to_Practice.frameNStart = frameN  # exact frame index
            Transition_to_Practice.tStart = t  # local t and not account for scr refresh
            Transition_to_Practice.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Transition_to_Practice, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Transition_to_Practice.started')
            # update status
            Transition_to_Practice.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(Transition_to_Practice.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(Transition_to_Practice.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if Transition_to_Practice.status == STARTED and not waitOnFlip:
            theseKeys = Transition_to_Practice.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            _Transition_to_Practice_allKeys.extend(theseKeys)
            if len(_Transition_to_Practice_allKeys):
                Transition_to_Practice.keys = _Transition_to_Practice_allKeys[-1].name  # just the last key pressed
                Transition_to_Practice.rt = _Transition_to_Practice_allKeys[-1].rt
                Transition_to_Practice.duration = _Transition_to_Practice_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                win=win, 
                timers=[routineTimer, globalClock], 
                currentRoutine=JawClench_Alignment,
            )
            # skip the frame we paused on
            continue
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            JawClench_Alignment.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in JawClench_Alignment.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "JawClench_Alignment" ---
    for thisComponent in JawClench_Alignment.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for JawClench_Alignment
    JawClench_Alignment.tStop = globalClock.getTime(format='float')
    JawClench_Alignment.tStopRefresh = tThisFlipGlobal
    thisExp.addData('JawClench_Alignment.stopped', JawClench_Alignment.tStop)
    # check responses
    if Transition_to_Practice.keys in ['', [], None]:  # No response was made
        Transition_to_Practice.keys = None
    thisExp.addData('Transition_to_Practice.keys',Transition_to_Practice.keys)
    if Transition_to_Practice.keys != None:  # we had a response
        thisExp.addData('Transition_to_Practice.rt', Transition_to_Practice.rt)
        thisExp.addData('Transition_to_Practice.duration', Transition_to_Practice.duration)
    thisExp.nextEntry()
    # the Routine "JawClench_Alignment" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisPractice_Loop in Practice_Loop:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        PT_EC_S1.setSound('/Users/tonysaldana/Downloads/Neurotech 25-26/ProstheticArm/Images&Sounds/beep-07a.mp3', secs=1.0, hamming=True)
        PT_EC_S1.setVolume(1.0, log=False)
        PT_EC_S1.seek(0)
        PT_EO_S2.setSound('/Users/tonysaldana/Downloads/Neurotech 25-26/ProstheticArm/Images&Sounds/beep-07a.mp3', secs=1.0, hamming=True)
    ...
    for thisBlock_Loop in Block_Loop:
        ...
        for thisTrial_Loop in Trial_Loop:
            ...
            continueRoutine = True
            # update component parameters for each repeat
            RT_EC.setSound('/Users/tonysaldana/Downloads/Neurotech 25-26/ProstheticArm/Images&Sounds/beep-07a.mp3', secs=1.0, hamming=True)
            RT_EC.setVolume(1.0, log=False)
            RT_EC.seek(0)
            RT_EO.setSound('/Users/tonysaldana/Downloads/Neurotech 25-26/ProstheticArm/Images&Sounds/beep-07a.mp3', secs=1.0, hamming=True)
            ...
            while continueRoutine and routineTimer.getTime() < 34.0:
                ...
                if RT_EC.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    ...
                    marker_outlet.push_sample(['Sound_EyesClosed'])
                ...
                if RT_EO.status == NOT_STARTED and tThisFlip >= 17.0-frameTolerance:
                    ...
                    marker_outlet.push_sample(['Sound_EyesOpen'])
